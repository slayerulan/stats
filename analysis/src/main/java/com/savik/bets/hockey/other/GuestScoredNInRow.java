...
@Getter
public class GuestScoredNInRow extends GeneralBet<HockeyMatch> {

    private int n;

    public GuestScoredNInRow(int n) {
        this.n = n;
    }
    ...
    public boolean check(HockeyMatch hockeyMatch) {
        ...
        for (HockeyGoal hockeyGoal : goalList) {
            if (hockeyGoal.getWhoScored() == Who.GUEST) {
                counter++;
            } else if (hockeyGoal.getWhoScored() == Who.HOME) {
                counter = 0;
            }
            if (counter == n) {
                return true;
